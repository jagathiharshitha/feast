...
class OciQueryOfflineStoreConfig(FeastConfigBaseModel):
    """Offline store config for OCI Query"""

    type: Literal["ociquery"] = "ociquery"
    """ Offline store type selector"""

    data_source: StrictStr
    """ oci query data source ex) OciObjectStorage """

    region: StrictStr
    """ OciQuery's OCI region """

    database: StrictStr
    """ OciQuery database name """

    workgroup: StrictStr
    """ OciQuery workgroup name """

    oss_staging_location: StrictStr
    """ OSS path for importing & exporting data to OciQuery """


class OciQueryOfflineStore(OfflineStore):
    @staticmethod
    @log_exceptions_and_usage(offline_store="ociquery")
    def pull_latest_from_table_or_query(
        ...
        assert isinstance(data_source, OciQuerySource)
...
class OciQueryRetrievalJob(RetrievalJob):
    def __init__(
            self,
